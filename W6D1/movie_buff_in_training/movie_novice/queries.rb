...
def below_average_years
  ...
  Movie.select('yr, COUNT(*) AS bad_movies').where('score < 5').group('movies.yr').order('yr DESC')
end
...
def pulp_fiction_actors
  ...
  Actor.select('actors.id, name').joins(:movies).where('title like ?', '%Pulp Fiction%')
end
...
def uma_movies
  ...
  Movie.select('movies.id, title, yr').joins(:actors).where('name like ?', '%Uma Thurman%')
end
